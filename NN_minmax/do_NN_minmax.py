...
def _getCategoricalFeatures ():
    ...
    categoricalFeaturesFile="categoricalFeatures.txt"
...
def _getMinmaxFeatures ():
    """The categorical features are stored in an external file"""
    MinmaxFeaturesFile="minmaxScalerFeatures.txt"

    MinmaxFeatures =[]
    fi = open(MinmaxFeaturesFile, "r")
    for line in fi :
        line = line.rstrip()
        MinmaxFeatures.append(line)
...
def _getXy (myFile) :
  ...
  #of course the last feature is the category
    ...
    MinmaxFeatures = _getMinmaxFeatures()
    ct = ColumnTransformer(
      [('one_hot_encoder', OneHotEncoder(categories='auto'), categoricalFeatures),
      ('min_max_scaler', MinMaxScaler(), MinmaxFeatures)],
...
REPEATS = range(10) #how many time a k-fold is run. How many averages will I see
TEST_NAME = 'NN_minmax' #name of this test, used for saving things.
KFOLD = 7 #ca 2900 in test set (split made on 0.14)
EPOCHS = 25 #epochs of each model.
...
def do_model(train_i,test_i):
    ...
    input_dim = 268 #num of features
    elu_alpha = 1.0
    seq = Sequential()

    seq.add(Dense(268, input_shape=(input_dim,), kernel_regularizer=regularizers.l2(0.01)))
    seq.add(Dropout(0.2))
    ...
    seq.compile(loss='binary_crossentropy', optimizer=adagrad, metrics=['accuracy']) 
...
for i in REPEATS:
    ...
    X_new = random.sample(X_dataneg, 51000)+X_datapos+X_datapos+X_datapos+X_datapos+X_datapos+X_datapos+X_datapos+X_datapos+X_datapos+X_datapos #no need to shuffle, kfold does it.
    y_new = random.sample(y_dataneg, 51000)+y_datapos+y_datapos+y_datapos+y_datapos+y_datapos+y_datapos+y_datapos+y_datapos+y_datapos+y_datapos
