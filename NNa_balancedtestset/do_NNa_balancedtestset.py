...
X_val_pos = X_datapos[-1135:]
y_val_pos = y_datapos[-1135:]
X_datapos = X_datapos[:-1135]
y_datapos = y_datapos[:-1135]
X_val_neg = X_dataneg[-1135:]
y_val_neg = y_dataneg[-1135:]
X_dataneg = X_dataneg[:-1135]
y_dataneg = y_dataneg[:-1135]

X_val = X_val_pos + X_val_neg #test set
y_val = y_val_pos + y_val_neg #test set labels
...
REPEATS = range(8) #how many time a k-fold is run. How many averages will I see
TEST_NAME = 'NNa_balancedtestset' #name of this test, used for saving things.
...
f.write(str(round(cm[1][0], 2)) + ' '+ str(round(cm[1][1], 2)))
f.close()
