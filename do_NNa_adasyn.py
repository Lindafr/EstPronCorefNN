...
X_data =load_sparse_csr('X_data')
X_val = load_sparse_csr('X_val')
y_val = load_y('y_val')
y_data = load_y('y_data')
...
TEST_NAME = 'NNa_adasyn' #name of this test, used for saving things.
...
for i in REPEATS:
    ...
    X_new = random.sample(X_dataneg, 42400)+X_datapos+X_datapos+X_datapos+X_datapos+X_datapos #no need to shuffle, kfold does it.
    y_new = random.sample(y_dataneg, 42400)+y_datapos+y_datapos+y_datapos+y_datapos+y_datapos
